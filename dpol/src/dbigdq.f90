...
subroutine ham(psi, psi2, e, amm, loop, md, lder, ldump, kdiv, bscale)
      ...
      do 22 nhi=1, nbx
         ...
         nbx2 = nblkx(nhi+1)
            ...
            do 22 nhj=1, nhi
               ...
               do 22 nbl1=nbx1, nbx2
                  if (nhi.eq.nhj) nbx4 = nbl1
                  nb1 = nblk(nbl1) + 1
                  nb2 = nblk(nbl1+1)

                  do 22 nbl2=nbx3, nbx4
                     nb3 = nblk(nbl2) + 1
                     nb4 = nblk(nbl2+1)
                    ...
                    500 if ((nb1.gt.1.and.nb3.gt.1).or.nh.eq.0) call cross(ld1(nb3), ld2(nb3), lx1, lx2, lrgl, id)
                        ...
                        if (loop.le.0 .and. .not.lder) then
                           write(4, 803) ld1(nb3), ld2(nb3), lx1, lx2, lrgl, id
                       803 format('ANGULAR COEFFICIENTS FOR', 6i3)
                           do k=1, 11
                              i1 = lmin(k)
                              i2 = lmax(k)
                              if (i2.eq.0) cycle
                              ...
                              write(4, 802) k, i1, i2,(cpl16(i, k), i=i1, i2, 2)
                           end do
                       802 format(3i3, 5d16.8)
                        end if
                        ...
                        do 23 j=nb3, i1
                           ...
                           if (.not. lder) then
                              ...
                              if (j.gt.nh) then
                                 ...
                                 !! THE NEXT 100 OR SO LINES OF CODE ARE UNTESTED AS OF AUG. 8, 2023. 
                              ...
                              if (namm.ne.0) then
                                 ...
                                 if (i.le.1 .and. j.le.1) then
                                    ...
                                    if (namm.gt.0) then
                                       ...
                                       830 continue
                                       sumq16=sumq
                                       if (abs(sumq).gt.1.q99) write(6, 829) i, j, pp, qq, ss, sumq16
                                       829   format(2i3,' SUMQ =', 3i4, 5d15.7)
                                       ss = ss - 1
                                    endif
                                    if (ldd1.ne.1.or.ss-ms1.le.3) go to 34
                                    !CCCCCCCCCCCCCCCCCC
                                    go to 34
                                    ipp = pp - mp1
                                    iqq = qq - mq1
                                    ...
                                    sum = 0.q0
                                    do 33 n=1, neig
                                    33 sum = sum + 2.q0*sx*dd1*ch(n)*((txx(ipp+1, iqq+n, iss-1, 2)            &
                                    &    - txx(ipp+1, iqq+n-1, iss, 2))*2.q0/z - e*t(ipp+1, iqq+n, iss))
                                    sum = sum - sx*sumq
                                    psi2(j1) = psi2(j1) + sum
                                    sum = 0.q0
                                    do 28 n=1, neig
                                    28 sum = sum + 2.q0*sx*dd1*ch(n)*t(ipp+1, iqq+n, iss)
                                    ovb(mdij) = ovb(mdij) + sum
                                    go to 29
                                    34 sum = fpl(pp, qq, 1) - yx
                                 ...
                                 sumq16=sumq
                                 yx = 2.q0*fpl(pp, qq, 1)
                                 !      if (I.EQ.1.AND.J.EQ.1.AND.ID.EQ.0) YX = 2.q0
                                 psi2(j1) = psi2(j1) + sx*(4.q0*sum/z - e*yx - sumq)
                                 ovb(mdij) = ovb(mdij) + sx*yx
                              end if
                              29 if (abs(psi2(j1)).gt.1.q80.or.abs(ovb(mdij)).gt.1.q75)           &
                              &   then
                              !TEMP1=PSI2(J1)
                              !TEMP2=OVB(MDIJ)
                              !TEMP3=SUMQ
                              !   WRITE(*, 670) J, I, PP, QQ, SS, TEMP1, TEMP2, TEMP3
                              !  WRITE(*, 670) J, I, PP16, QQ16, SS16, PSI2(J1), OVB(MDIJ), SUMQ16
                              670 format(5i3, 1p, 4d16.9/15x, 4d16.9)
                              !   READ(*,*)
                              endif
                           ...
                           nspm = 2*(nspx - nsm)
      !      WRITE(*,*) 'Calculating derivatives - hit ENTER'
      !read(*,*)
                           nspp = 2*(nspx + nsm)
                     !   (1, 1) = AJ, (2, 1) = BJ, (1, 2) = AI, (2, 2) = BI, WHERE AJ MEANS ALPHA(J) ETC.
                           d2d(1, 1) = nspm*(npj+1)
                           d2d(2, 1) = nspm*(nqj+1)
                           ...
                           iqsp2 = iqsp + 2
                           sum = 0.q0
                           if (j.le.nh) go to 602
                     637 if (.not.lr12) go to 40
                     !
                     ...
                     !   LEADING TERMS SUBTRACTED.  ONLY 'DIRECT' TERMS ARE CONSIDERED.
                        ...
                        &   - cb2*dqreal(iqsp2)))
                           ...
                           if (ipow.eq.2) go to 79
                           tt(1) = 2.q0*bpx*(bpx*dqreal(nqsm) - bmx*dqreal(iqsp1))
                           tt(2) = tt(6) + bp2x
                           go to 78
                        79 tt(1) = 2.q0*apx*(apx*dqreal(npm) - amx*dqreal(ipp1))
                           tt(2) = tt(5) + ap2x
                        78 continue
                           if (ss.eq.2) go to 82
                           if (lgo.and.ipow.eq.1) go to 70
                           nsp1 = nsp + 1
                           do 74 if=1, 3
                        ...
                        73 sumr = d(1)*fddr(1, 1)+d(2)*fddr(2, 1)+(d(3)-ee2)*fddr(3, 1)/dqreal(nl22) &
                        &   +d(4)*fddr(1, 2)+d(5)*fddr(2, 2)+d(6)*fddr(3, 2)
                           go to 80
                        82 if (ipow.eq.2) go to 80
                           df1x(1, 1) = 0.q0
                           df1x(2, 1) = 0.q0
                           df1x(1, 2) = 0.q0
                           df1x(2, 2) = 0.q0
                     !   80 ZR12 = (TXX(IPP1, IQP+1, NSP, 2) - TXX(IPP1, IQP, NSP+1, 2))*(4./Z)
                        80 zr12 = (at(jat(pp, qq)+ss-1) &
                        &    - at(jat(pp, qq-1)+ss))*(4.q0/z)
                           hmxx(ipow) = sumr + zr12*dqreal(nspx)
                           if (lblk2) go to (559, 84), ipow
                           if (ipow.eq.2) go to 83
                           df1x(2, 1) = df1x(2, 1) + fdd30*(tt(5)/tt(3) + (tt(2) + tt(1))/tt(4) &
                        &    + e10)
                           df1x(idex, 2) = df1x(idex, 2) + fdd30*(tt(5)/tt(3) &
                        &    + (tt(2) - tt(1))/tt(4) + e10)
                           go to 559
                        83 df1x(1, 1) = df1x(1, 1) + fdd30*((tt(2) + tt(1))/tt(3) + tt(6)/tt(4) &
                        &    + e10)
                           df1x(ide, 2) = df1x(ide, 2) + fdd30*((tt(2) - tt(1))/tt(3) &
                        &    + tt(6)/tt(4) + e10)
                        84 dhmx(nhi, 2) = dhmx(nhi, 2) + 0.5q0*(-hmxx(ide) + df1x(2, 2))*ps2
                     ...
                     559 continue
                           if (namm.eq.0) go to (700, 23), ipow
                     !
                           if (ipow.eq.1) go to 814
                     ...
                     !
                           ...
                           go to 700
                     !
                        40 if (lgo.and.ipow.eq.1) go to 44
                           fdd(3) = fpl(pp, qq, 1)
                        ...
                        35 ss = ss - 2
                           fdd(6) = fpl(pp, qq, 1)
                           ss = ss + 2
                        36 if (ipow.eq.1) go to 42
                           fdd(2) = fdd(5)
                           fdd(5) = fpl(pp, qq-1, 1)
                           df1(1, 1) = df1(1, 1) + d2d(1, 1)*fdd(1)
                           if (id.eq.1) go to 112
                           df1(1, 2) = df1(1, 2) + d2d(1, 2)*fdd(1)
                           go to 43
                     112 df1(2, 2) = df1(2, 2) + d2d(2, 2)*fdd(1)
                           go to 43
                        42 fdd(2) = fpl(pp-1, qq, 1)
                           fdd(5) = fpl(pp, qq-1, 1)
                        44 df1(1, 1) =              d2d(1, 3)*fdd(5)
                           df1(2, 1) = d2d(2, 1)*fdd(4) + d2d(2, 3)*fdd(5)
                           if (id.eq.1) go to 113
                           df1(1, 2) =              d2d(1, 4)*fdd(5)
                           df1(2, 2) = d2d(2, 2)*fdd(4) + d2d(2, 4)*fdd(5)
                           go to 114
                     113 df1(1, 2) = d2d(1, 2)*fdd(4) + d2d(1, 4)*fdd(5)
                           df1(2, 2) =              d2d(2, 4)*fdd(5)
                     114 if (lgo) go to 71
                        43 ss = ss - 1
                     !      if (SS.GT.1) GO TO 151
                     !      ZR12 = 4.*FPL(PP, QQ, 2)/Z
                     !      GO TO 72
                     151 if (.not.ltxx)zr12 = 4.q0*(fpl(pp, qq, 1) - fdd(5))/z
                           if (ltxx) zr12 = (txx(ipp1, iqp+1, nsp, 2) - &
                        &                    txx(ipp1, iqp, nsp+1, 2))*(dqreal(2.q0)/z)
                     !   THE ABOVE COMES FROM  2*(1/R12 - 1/R2).
                        72 ss = ss + 1
                        71 sum = zr12*dqreal(nspx) + (d(1)*fdd(1) + d(2)*fdd(2) + (d(4)*fdd(4)+d(5) &
                        &   *fdd(5)) + (d(3)-ee2)*fdd(3)/dqreal(nl22) + d(6)*fdd(6))
                     !   THE ABOVE ARISE FROM TERMS INDEPENDENT OF CORRELATION.
                     !   INCLUDE LOGARITHMIC CONTRIBUTIONS.
                           if (nsp.eq.0.and.nsj.ne.nsi) then
                           lg = 0
                           nsm = nsj - nsi
                           ...
                           fddl5 = fpl(pp, qq-1, 1)
                           fddl3 = fpl(pp, qq, 1)
                           sum = sum + dl1*fddl1 + dl2*fddl2 + dl3*fddl3/dqreal(nl22) + dl4*fddl4 &
                        &     + dl5*fddl5
                           lg = 1
                           endif
                     !
                     !   GENERAL FORM OF MASS POLARIZATION CONTRIBUTIONS.
                           if (namm.eq.0) go to 208
                           if (ipow.eq.1) go to 809
                           ...
                           if (id.eq.1) go to 811
                           df1(1, 2)= df1(1, 2) + fqq4 - bi*fqq2 + cips*dqreal(npp+npi+2)*fdd(1)
                           df1(2, 2) = df1(2, 2) + fqq3 - ai*fqq2
                           go to 813
                     811 df1(2, 2)= df1(2, 2) + fqq4 - bi*fqq2 + cips*dqreal(npp+npi+2)*fdd(1)
                           df1(1, 2) = df1(1, 2) + fqq3 - ai*fqq2
                           go to 813
                     809 df1(1, 1) = df1(1, 1) - ai*cips*fdd(5)
                           df1(2, 1) = df1(2, 1) - cips*(bi*fdd(5) - dqreal(nqi)*fdd(4))
                           if (id.eq.1) go to 812
                           df1(1, 2) = df1(1, 2) - (ai+ap)*cips*fdd(5)
                           df1(2, 2) = df1(2, 2) - cips*((bi+bp)*fdd(5) - dqreal(nqp+nqi+2)*fdd(4))
                           go to 813
                     812 df1(2, 2) = df1(2, 2) - (ai+ap)*cips*fdd(5)
                           df1(1, 2) = df1(1, 2) - cips*((bi+bp)*fdd(5) - dqreal(nqp+nqi+2)*fdd(4))
                     813 sumq = abi*fpl(pp, qq, 5) + fpl(pp-1, qq-1, 9) - bi*fpl(pp-1, qq, 10) &
                        &   - ai*fpl(pp, qq-1, 11)
                           if (ldd4.gt.0)sumq = sumq - cip*(fpl(pp-2, qq, 4)+fpl(pp, qq-2, 4))
                           sumq = sumq + qd1*fdd(1) + qd2*fdd(2) + qe1*fdd(4) + qe2*fdd(5) &
                        &    + qd3*fdd(3) + qd4*fdd(6)
                     208 hmx(ipow) = sum - sumq*dqreal(nspx)
                           go to 700
                     602 yx = fpl(pp, qq-1, 1)
                           ss = ss - 1
                           if (i.gt.1.or.j.gt.1) go to 620
                           sum = fpl(pp, qq, 2)
                     ...
                     620 ss = ss + 1
                           sumq = amm4*(aj*bj*fpl(pp, qq, 5) - aj*fpld(pp, qq-1, 5))
                     !
                           do 833 l=mldd5, ldd5, 2
                           cpl(l, 9) = amm4*(dqreal(npj*nqj)*cpl(l, 5) + dqreal(npj)*cpl(l, 8) + &
                        &   dqreal(nqj)*cpl(l, 7) + cpl(l, 6))
                           cpl(l, 10) = amm4*(dqreal(npj)*cpl(l, 5) + cpl(l, 7))
                     833 cpl(l, 11) = amm4*(dqreal(nqj)*cpl(l, 5) + cpl(l, 8))
                           do 834 k=9, 11
                           lmin(k) = mldd5
                           lmax(k) = 0
                           do 834 l=mldd5, ldd5, 2
                           if (abs(cpl(l, k)).lt.1.q-10) cpl(l, k) = 0.q0
                           if (cpl(l, k).ne.0.q0) lmax(k) = l
                     834 continue
                     !
                           ...
                           if (ldd10.gt.0) sumq = sumq - bj*fpl(pp-1, qq, 10)
                           if (ldd11.gt.0) sumq = sumq - aj*fpl(pp, qq-1, 11)
                     630 if (abs(sumq).gt.1.q90) write(6, 829) i, j, pp, qq, ss, sumq
                           ss = ss - 1
                           if (ldd1.ne.1) go to 621
                           ipp = pp - mp1
                           iqq = qq - mq1
                           sum = 0.q0
                           do 622 n=1, neig
                     622 sum = sum + 2.q0*dd1*ch(n)*((txx(ipp+1, iqq+n, ss-1, 2) &
                        &    - txx(ipp+1, iqq+n-1, ss, 2))*2.q0/z - e*t(ipp+1, iqq+n, ss))
                           hmx(ipow) = (sum - sumq)*dqreal(nspx)
                           ss = ss + 1
                           go to 623
                     621 sum = fpl(pp, qq, 1) - yx
                     629 ss = ss + 1
                           yx = 2.q0*fpl(pp, qq, 1)
                     ...
                     623 do 90 if=1, 2
                           do 90 jf=1, 2
                        90 df1(if, jf) = 0.q0
                     !  700 if (ABS(HMX(IPOW)).GT.1.q80) WRITE(6, 670)J, I, PP, QQ, SS, HMX(IPOW)
                     700 continue
                     !  700  WRITE(4, 670)J, I, PP, QQ, SS, HMX(IPOW), DF1(1, 1), DF1(2, 1), DF1(1, 2),
                     !     1   DF1(2, 2), SUMQ
                     ...
                     !write(*,'(2i4, 4d15.7)') nhi, nhj, qreal(dhm(nhi, 1)), qreal(dhm(nhj, 1)), qreal(ps2)
                     !C
                     23 continue
                  !read(*,*)
                        if (lder) go to 22
                     91 do 92 j=nb3, i1
                     92 hm(md(i)+j) = psi2(j-nb3+1)
      22 continue
   !   EMPTY REMAINING OVERLAP INTEGRALS IN OVB.
         if (lder) go to 122
         if (ldump) then
         if (idxx.eq.0) nrow(ndmp) = irow
   ...
   !CCCCCCCCCCCCCC REMOVE
   !C      SC = SC/10
   !CCCCCCCCCCCCCC
         cpd(i, k) = cpd(i, k) - sign(sc*abs(cpd(i, k)), dhmk)
   !write(*,'(a, 2i4, d15.7)') 'CPD =', i, k, qreal(cpd(i, k))
      102 continue
         kst = 1
         kstder = 1
         go to 108
   ...
   !C      if (QABS(DIFF).LT.SC.OR.QABS(DHMK).LT.1.D-18)
         if (abs(diff).lt.sc.or.abs(dhmk).lt.1.q-26/bb) &
         &    diff = sign(sc, diff)
         if (i.eq.1.and.k.eq.1.and.iset.eq.1) diff = 0.q0
   !C   CHECK FOR CONVERGENCE OF DERIVATIVES
         ...
         b(i, k) = bb
      104 der(i, k) = dhmk
   !C      if (QABS((ENEW-E)/E).LT.0.5D-15) KST = 0
         if (abs((enew-e)/e).lt.0.5q-16) kst = 0
         if (kstder.eq.0) kst = 0
         if (kstder.eq.10) kst = 1
      108 write(*, 107) qreal(e), qreal(enew), qreal(e-enew)
         write(4, 107) qreal(e), qreal(enew), qreal(e-enew)
      107 format(' Eold =', f30.24/' Enew =', f30.24/' Diff ='f30.24)
         call reset(dhm, e, 0, bscale)
         write(*, 705) loop,((qreal(dhm(i+nh, k)), k=1, 2), i=1, nbet)
         write(4, 705) loop,((qreal(dhm(i+nh, k)), k=1, 2), i=1, nbet)
      705 format(' LOOP =', i3,', DERIVATIVES ARE'/(1p8d12.4))
         write(*, 106) ((qreal(b(i, k)), k=1, 2), i=1, nbet)
         write(*, 106) ((qreal(cpd(i, k)), k=1, 2), i=1, nbet)
         write(4, 106) ((qreal(b(i, k)), k=1, 2), i=1, nbet)
         write(4, 106) ((qreal(cpd(i, k)), k=1, 2), i=1, nbet)
      106 format(8f12.5)
   !C      DO 105 I=1, NBET
   !C      DO 105 K=1, 2
   !C      SC = 10000.
   !C      if (CPD(I, K).LT..1Q0) SC = 100000.
   !C      J  = SC*(CPD(I, K)+.5/SC)
      105 cpd(i, k) = dqreal(j)/sc
         do 199 i=1, 10
      write(4,'(''-------------------------------------------------'')')
   199 continue
   !   199 BACKSPACE 6
...
if (l2.eq.0) lam2a = 2
bl1 = dqreal(l1 + 1)
do 50 i=lam1a, lam1b, 2
lm1 = i - 1
if (i.eq.lam1b) bl1 = dqreal(-l1)
bl2 = dqreal(l2 + 1)
do 50 j=lam2a, lam2b, 2
lm2 = j - 1
if (j.eq.lam2b) bl2 = dqreal(-l2)
if (lm1+lm2.lt.l.or.iabs(lm1-lm2).gt.l) go to 50
...
if (lcapa.gt.lcapb) go to 50
do 51 k=lcapa, lcapb, 2
lc = k - 1
...
if (lmin1.gt.lmax1) go to 53
do 52 i=lmin1, lmax1, 2
lc = i - 1
...
do 32 i=1, lmax5
j = lmax5 - i + 1
if (abs(cpl(j, k)).gt.1.q-10) go to 33
cpl(j, k) = 0.q0
32 end do
33 lmax(k) = j
do 30 i=1, lmax5
if (abs(cpl(i, k)).gt.1.q-10) go to 31
cpl(i, k) = 0.q0
30 end do
lmax(k) = 0
i = 0
31 lmin(k) = i
if (lmax(k).gt.9) go to 102
36 end do
do 37 k=1, 4
do 38 i=lmin1, lmax1, 2
if (k.eq.2.and.i.eq.1) go to 38
csum(k) = csum(k) + cpl(i, k)
38 end do
if (abs(csum(k)).lt.1.q-10) csum(k) = 0.q0
37 continue
do 43 k=5, 8
do 44 i=lmin5, lmax5, 2
csum(k) = csum(k) + cpl(i, k)
44 continue
if (abs(csum(k)).lt.1.q-10) csum(k) = 0.q0
43 continue
return
102 write(6, 101) lmax(k)
101 format(' ERROR IN CROSS, LMAX = ', i4)
stop
end
!****************************************
...
if (isig)201, 200, 202
200 outfac = outfac*del2vw(id, ie, ic, isig)
if (isig)201, 203, 202
203 outfac = outfac*del2vw(id, ib, if, isig)
if (isig)201, 204, 202
204 outfac = outfac*del2vw(ia, ie, if, isig)
if (isig)201, 205, 202
205 outfac =sqrt(outfac)
ipos(1) =     ia+ib+ic
ipos(2) =     id+ie+ic
ipos(3) =     id+ib+if
ipos(4) =     ia+ie+if
...
201 f6j    = 0.0q0
return
202 write(4, 230) isig
230 format("36h ***** fault in call of wbar01, isig=, i2")
f6j    = 0.0q0
return
end
!*****************************************************
...
!         ISIG =+1, AN ANGULAR MOMENTUM ARGUEMENT NEGATIVE,(WRITE)
!         ISIG =+2, SUM OF ARGUEMENTS NON-INTEGRAL,(WRITE)
!         ISIG = 0, OTHERWISE AND DEL2VW IS EVALUATED.
!
common/a1a/fac(91), r(1250), y(84), yp(84), y1x(84), y2x(84), z1x(84)   &
&  , ts(3, 3, 2), ermac, alf, bet, xap, xbp, xam, xbm, suml(86, 2, 6), fp2(84)   &
&  , fm2(84), f21x(84, 86)
!
if ((ia.ge.0).and.(ib.ge.0).and.(ic.ge.0))go to 500
write(4, 531)
531 format(43h ***** negative angular momentum in del2vw.)
isig = +1
return
500 ism = ia+ib+ic
if ((ia.ge.iabs(ib-ic)).and.(ia.le.ib+ic))go to 502
isig = -1
return
502 k1 =     ia+ib-ic
k2 =     ib+ic-ia
...
!***********************************************************
subroutine eigall(nrow, norder, matrix, eigval, option, eigvec, dpwork, &
&                  ierr)
!
use dqmodule
implicit type (dq_real) (a-h, o-z)
...
integer            nrow, norder, option, ierr, j, imax, i, i1, nv
!               FIRST EXECUTABLE STATEMENT - CHECK INPUT.
if (option .ne. 0) go to 110
!------------------------ FIND EIGENVALUES ONLY ------------------------
!               COPY MATRIX INTO THE FIRST NORDER*(NORDER+1)/2 STORAGE
!               LOCATIONS IN DPWORK.
j = 1
...
!               TRANSFORM THE REAL SYMMETRIC MATRIX WHOSE EIGENVALUES
!               ARE WANTED TO TRIDIAGONAL FORM VIA ORTHOGONAL
!               SIMILARITY TRANSFORMATIONS.
nv = norder*(norder+1)/2
call tred3(norder, nv, dpwork(1), eigval, dpwork(i1), dpwork(i1))
!               DETERMINE THE EIGENVALUES OF THE SYMMETRIC
!               TRIDIAGONAL MATRIX USING THE IMPLICIT QL
!               METHOD.
...
if (norder .gt. nrow) go to 170
!               COPY MATRIX INTO EIGVEC.
k = 1
do 130 i = 1, norder
   do 120 j = 1, i
      temp = matrix(k)
...
!               OF THE SYMMETRIC TRIDIAGONAL MATRIX USING
!               THE IMPLICIT QL METHOD, AND BACKTRANSFORM
!               THE EIGENVECTORS TO THE ORIGINAL BASIS.
call imtql2(nrow, norder, eigval, dpwork, eigvec, ierr)
if (ierr .ne. 0) go to 190
return
!               ERROR RETURNS.  FIRST THE FORMAT STATEMENTS.
140 format(/' FATAL ERROR IN SUBROUTINE EIGALL.  SUBROUTINE TERMINATED&
& DUE TO IMPROPER INPUT:'//' NORDER .GT. NROW.'/)
150 format(/' FATAL ERROR IN SUBROUTINE EIGALL.  SUBROUTINE TERMINATED&
& DUE TO EISPACK ERROR:'//' IERR SET EQUAL TO ', i10,' BY IMTQL1.'/)
160 format(/' FATAL ERROR IN SUBROUTINE EIGALL.  SUBROUTINE TERMINATED&
& DUE TO EISPACK ERROR:'//' IERR SET EQUAL TO ', i10,' BY IMTQL2.'/)
!               ERROR RETURN WRITE STATEMENTS.
170 continue
write(6, 140)
return
!
...
dimension d(n), e(n)
!REAL*16 B, C, F, G, P, R, S, MACHEP
!REAL*16 SQRT, ABS, SIGN
!
...
!*    MACHEP = 2.0Q0**(-75)
!
!     DETERMINE MACHEP AND NEPS.  MACHEP, KNOWN AS MACHINE EPSILON, IS
!     THE SMALLEST NUMBER SUCH THAT MACHEP+1 IS NOT EQUAL TO 1 IN THE
!     FINITE PRECISION ARITHMETIC USED BY THE COMPUTER.  NEPS IS AN
!     INTEGER SUCH THAT MACHEP = (1/2)**NEPS.
!
...
!      QUADRUPLE PRECISION F, G, H, HH, SCALE
!REAL*16 SQRT, ABS, SIGN
!
...
!      QUADRUPLE PRECISION F, G, H, HH, SCALE
!     QUADRUPLE PRECISION QSQRT, QABS, QSIGN
!
...
!
!        A CONTAINS THE LOWER TRIANGLE OF THE QUADRUPLE PRECISION SYMMET
...
!write(*,*) qreal(ovb(mix)), qreal(sh(mi)), qreal(SUM)
!write(*,*) qreal(psi(i)), qreal(psi(j))
!read(*,*)
sum = sum + ps2*sh(mi)
331 continue
sum = sum + egs
...
use dqmodule
implicit type (dq_real) (a-h, o-z)
!   SET KHM = 1 FOR HAMILTONIAN MATRIX ELEMENTS AND 0 FOR OVERLAP INTEGR
common/s1s/faca(80), facb(80), ngv(100, 10), ng(100),                 &
&   igrp(698), ngp, nngt
dimension th(14, 14), tth(14, 14), u(14, 14)                           &
&   , psi(1), h(1), md(12150)
!
!
!   GENERATE UNITARY TRANSFORMATION TO DIAGONALIZE MATRIX OF ALL 1's.
!   TRANSFORM EIGENVECTOR IF ITT = 1
!   APPLY INVERSE TRANSFORM TO MATRIX IF ITT < 0.
202 nng = nngt
do 33 j=2, nng
...
34 end do
33 u(j, j) = -dqreal(j-1)*uj
!
if (itt.eq.1) go to 50
do 30 i=1, ngp
nng = ng(i)
do 31 j=1, nng
jv = ngv(i, j)
...
use dqmodule
implicit type (dq_real) (a-h, o-z)
!   UPDATE THE INPUT FILE FOR PROGRAM RE-START.
integer pam, qam
...
!C   COMMENTED OUT FEB. 5, 2021.
!C      if (ISET.EQ.1.AND.KBASIS.EQ.1) CPD1(2) = 1.Q0
...
do 11 i=4, 2000
ii = i - 1
read(3,'(A)', end=12) line(i)
end = line(i)(1:2)
if (end.ne.'0/') go to 11
if (iend.ne.0) go to 20
iend = i
go to 11
20 if (iend2.eq.0) iend2 = ii
11 end do
12 iend = max0(iend, iend2)
...
rewind 3
write(3,'(A)') line(1)
if (kst.ne.0) go to 117
do 14 i=4, iend
write(3,'(A)') line(i)
14 continue

117 nloop = max0(nit-next, 1)
if (kst.eq.0) nloop = nit
if (kontrol.ne.0) nloop = -kontrol
e16=e
write(3, 21) e16, nloop, neig
21 format(f28.25,',', i2,',', i2)
write(3, 27)iz, lrglx, nspn, l1max, fnwve, title(1:44)//status
27 format(i2,',', 3(i1,','),'''', a12,''',''', a52,'''')
key = 1
do i=1, nbet
  b16(i, 1)=b(i, 1)
  b16(i, 2)=b(i, 2)
...
if (bscale.gt.0.q0) cpd2(1) = cpd2(1)/bscale
if (key.eq.1) write(3, 23) (qreal(b(i, 1)), qreal(b(i, 2)), i=1, nbet)
if (key.eq.0) write(3, 23) (qreal(cpd1(i)), qreal(cpd2(i)), i=1, nbet)
23 format(f8.5, 9(',', f8.5))
write(3, 25) (ksm(i), i=1, nbetx), key, mar12, mar1, kono
25 format(10(i2,','))
if (key.eq.0) go to 17
nh0 = nh
if (dhm(nbet+nh, 1).eq.0.q0) nh0 = 0
write(3, 24)((qreal(dhm(i+nh0, k)), k=1, 2), i=1, nbet)
24 format(1p6d12.4)
do i=1, nbet
  do k=1, 2
    b16(i, k)=b(i, k)
...
write(3, 26)((qreal(b(i, k)), k=1, 2), i=1, nbet)
write(3, 26)(qreal(cpd1(i)), qreal(cpd2(i)), i=1, nbet)
26 format(6f12.5)
17 if (kst.ne.0) iend = 3
...
!   SUBROUTINE TO READ THE INCORRECTLY ORDERED OVERLAP INTEGRALS FROM FI
!   AND ARRANGE THEM IN THE CORRECT ORDER.  THE LOOP STRUCTURE MUST BE I
!   TO THAT IN 'HAM'.
use dqmodule
implicit type (dq_real) (a-h, o-z)
integer pam, qam
...
i1 = nb4
if (nb3.eq.nb1) i1 = i
if (key.eq.0) go to 25
if (md(i)+i.le.lngth) go to 22
stop 22
25 nadd = i1 - nb3 + 1
if (irow+nadd.lt.maxbf) go to 50
!   READ A BLOCK OF DIRECT + EXCHANGE INTEGRALS FROM FILE 9.
...
do 23 j=nb3, i1
ibf = j - ir1
23 sh(md(i)+j) = ovb(ibf)/(dso(i)*dso(j))
22 continue
return
end
!*******************************************************
...
integer pam, qam
!REAL *16 ESH16, CPD1X16, CPD2X16, CP116, CP216, CPD116, CPD216, PSI16, PSI216, &
!& SSH16, PSID16, TOT16, DSO16
dimension cpd1x16(8), cpd2x16(8), cp116(12150), cp216(12150), cpd116(8), cpd216(8)
common/r1r/rap, egs, ltrid
common/done/lr12, idone, lext
common/s1s/faca(80), facb(80), ngv(100, 10), ng(100),                 &
...
!      DO 554 I=1, NV
!      DO 554 J=1, I
!  554 WRITE(4, 557) I, J, FETCH(SH, MD(I)+J)
!  557 FORMAT(2I4, D15.7)
!      CALL SORT(SH, OVB, DSO, PSI2, NV, MD, 1)
esh = e
kout = 0
write(*, 609) -qreal(esh),(qreal(cpd1(i)), i=1, nbet)
609 format('E(0) =', f20.16,' ALPHA =', 5f10.6)
write(*, 650) qreal(psi(1)),(qreal(cpd2(i)), i=1, nbet)
call dqwrite(6, psi(1))
!read(*,*)
650 format('O(1) =', f20.16,'  BETA =', 5f10.6)
do 30 i=1, nv
!DSO(I) = SQRT(OVB(MD(I)+I))
      dso(i) = sqrt(psi(i))
...
31 continue
!LRGL      if (LRGL.NE.0.AND.NH.NE.0) CALL COMB(SH, PSI, NV, MD, 0, 1)
if (ltrans.ne.0) call comb(sh, psi, nv, md, 0, 1)
call addom(egs, sh, psi, md,-1)
!
if (ldump) then
!   STORE THE HAMILTONIAN MATRIX ELEMENTS (IN RY) IN FILE 8 AS A LINEARI
!   TRIANGULAR ARRAY OF THE FORM SH(MD(I)+J) WITH I .GE. J.
!        WRITE(4, 84)
!   84   FORMAT(1H0, 31HHAMILTONIAN MATRIX ELEMENTS ARE )
!        WRITE(4, 881) ((I, J, FETCH(SH, MD(I)+J), J=1, I), I=1, NV)
!  881   FORMAT(2(2I5, D15.7))

  rewind 8

  write(8) sh
!   RECOVER THE UNSORTED OVERLAP INTEGRALS FROM FILE 9 AND SORT THEM INT
...
!LRGL      if (LRGL.NE.0.and.nh.ne.0) CALL COMB(OVB, PSI, NV, MD, 0, 0)
  if (ltrans.ne.0) call comb(ovb, psi, nv, md, 0, 0)
  call addom(egs, ovb, psi, md, 0)
!        WRITE(4, 881) ((I, J, OVB(MD(I)+J), J=1, I), I=1, NV)
endif
!
if (.not.lpow) then
...
!   SAVE TRANSFORMED WAVE FUNCTION COEFS. IN 'PSI2' FOR USE IN 'ADDER'.
nbb2 = nb2
      do 32 i=1, nbb2
   32 psi2(i) = psi(i)
!LRGL      if (LRGL.NE.0.AND.NH.NE.0) CALL COMB(SH, PSI, NV, MD, 1, 0)
if (ltrans.ne.0) call comb(sh, psi, nv, md, 1, 0)
!      if (LEXT) CALL SHRINK(PSI, NV)
write(*, 52) next
write(4, 52) next
if (.not.lpow) write(*,*) 'E =', qreal(e)
52 format(57x,'  LOOP', i3)
do 33 i=1, 7
psid(i) = psi(i)
33 continue
write(*, 6) (qreal(psid(i)), i=1, 7)
write(4, 6) (qreal(psid(i)), i=1, 7)
6 format(' EIGENVECTOR IS'/1x, 1p, 7d10.3)
do 53 i=1, nv
psi(i) = psi(i)/dso(i)
53 continue
      if (lpow) return
...
2   format(1x, 1p3d26.19)
  write(6, 66)
66   format(12h eigenvalues )
  write(*, 2) (qreal(ee(i)), i=1, 10)
return
end
!***************************************************
...
equivalence (tt(1), t1),(tt(2), t2),(tt(3), t3),(tt(4), t4),(tt(5),   &
&   shift),(tt(6), scale)
equivalence (ttp(3), c2)
logical lext
!DATA TTP/0.q0, 1.Q0, 0.q0, 1.Q0, 0.q0, 0.q0/
ttp(1)=0.q0
ttp(2)=1.q0
ttp(3)=0.q0
ttp(4)=1.q0
...
!LRGL      if (LRGL.EQ.0.OR.NH.EQ.0) RETURN
...
nqsp = nqsi + nqsj
nqsm = nqsi - nqsj
!      OV = 2.*FACA(NPP+1)*FACB(NQSP+1)
!     1   /(PSI(I)*PSI(J))
!      OV = FAC(NPP+1)*FAC(NQSP+1)/QSQRT(FAC(2*NPI+1)*FAC(2*NPJ+1)
!     1   *FAC(2*NQSI+1)*FAC(2*NQSJ+1))
ja = min0(npi, npj)
jap = max0(npi, npj)
jb = min0(nqsi, nqsj)
jbp = max0(nqsi, nqsj)
...
!C      SCALE = 1
!*      if (.NOT.LEXT) GO TO 1001
!*      GO TO 69
1001 x1 = 1.q0
do 11 k=2*ja+3, ja+jap+2
x1 = x1*dqreal(k/2)
11 continue
x2 = 1.q0
do 12 k=ja+jap+3, 2*jap+2
x2 = x2*dqreal(k/2)
12 continue
x3 = 1.q0
do 13 k=2*jb+3, jb+jbp+2
x3 = x3*dqreal(k/2)
13 continue
x4 = 1.q0
do 14 k=jb+jbp+3, 2*jbp+2
x4 = x4*dqreal(k/2)
14 continue
ov = sqrt(scale*(x1*x3)/(x2*x4))
if (khm.eq.0) go to 63

ov = ov*((t1*t4+t2*t3)/(t2*t4) + shift)
!      if (QABS(OV/H(IH)).LT.10.)WRITE(4, 555)NPI, NQI, NSI, NPJ, NQJ, NSJ,
!     1   OV, H(IH), OV/H(IH)
!  555 FORMAT(3I3, 2X, 3I3, 3D15.7)
63 h(ih) = h(ih) + ov
69 if (khm.eq.0) go to 60
do 70 k=1, ngi
isk = k*(k-1)
...
72 ih = md(lv) + kv
74 continue
!*      if (.NOT.LEXT) GO TO 1002
!*      GO TO 75
1002 h(ih) = h(ih) + sqrt(dqreal(isk*isl)*x1*x3/(x2*x4))*c2/t4
75 end do
70 end do
60 continue
return
...
!   CALCULATE SQRT(OVERLAP INTEGRALS) FOR DIAGONAL DOMINANT TERMS.
!   THESE ARE USED TO NORMALIZE THE BASIS FUNCTIONS.
alf = 2.q0*cp1(2)
bet = 2.q0*cp2(2)
...
write(*, 1) nr
1 format(' ORTHONORMALIZATION COEFS. ARE', i10)
do 20 i=1, nv
...
write(*, 3) nr
3 format(' BASIS FUNCTION COEFS. ARE', i10)
do 10 i=1, nv
do 11 j=1, nv
tot(i, j) = 0.q0
11 continue
...
equivalence (jj(1), ja),(jj(2), jap),(jj(3), jb),(jj(4), jbp)
equivalence (tt(1), t1),(tt(2), t2),(tt(3), t3),(tt(4), t4),(tt(5),   &
&   shift),(tt(6), scale),(tt(7), t1a),(tt(8), t2a),(tt(9), t3b),      &
&   (tt(10), t4b)
!DATA TTP/0., 1.Q0, 1.Q0, 1.Q0, 0., 0., 0., 1.Q0, 0., 1.Q0/
ttp(1)=0.q0
ttp(2)=1.q0
ttp(3)=1.q0
ttp(4)=1.q0
...
!
!    FIND THE CLOSEST EIGENVALUE TO THE GUESS EGS USING
!     THE POWER METHOD.
!
use dqmodule
implicit type (dq_real) (a-h, o-z)
...
1 continue
x(1) = 1.q0
!   CONVERT 'X' ARRAY TO EXTENDED PRECISION FORMAT (12 BYTE).
e=0.q0
dxm = 1.q60
if (.not.ldump) then
  rewind(8)
...
44 kcyc = k
!   REPLACE 'C*' STATEMENTS BY EXTENDED PRECISION ROUTINES AS INDICATED.
!***************************************
!*      if (LEXT) GO TO 1001
do 22 i=1, n
y(i) = x(i)/x(1)
...
!***************************************
!*      if (LEXT) GO TO 1002
!L STATEMENTS ARE REPLACED BY YAX ROUTINE.
...
es=e
!***************************************
!  THE FOLLOWING RETURNS 'E' AND 'DXM' IN QUADRUPLE PRECISION AND 'EE' I
!  EXTENDED PRECISION.  'EE' MUST HAVE DIMENSION 2 TO ACCOMMODATE IT'S L
!*      if (LEXT) GO TO 1003
sum = 0.q0
sum2 = 0.q0
dxmp = dxm
...
et16=et
chk16=chk
write(*, 100) kcyc, dxm16, im, et16, chk16
write(4, 100) kcyc, dxm16, im, et16, chk16
100  format(' ITER =', i2,' DXM =', d10.3,' FOR', i4,' E =',             &
&   d32.25, d12.3)
if (kcyc.lt.2) go to 18
if (dxm.ge.dxmp) go to 40
if (dxm.lt.1.q-26) go to 40
18 end do
write(*, 8) ncyc
write(4, 8) ncyc
8 format(1h0,'SYSTEM HAS NOT CONVERGED AFTER ', i3, 7h cycles)
40 continue
41 continue
rewind 8
if (ldump) rewind 9
...
!***************************************
!***************************************
!   THE FOLLOWING RETURNS 'SUMH' AND 'DXM' IN QUADRUPLE PRECISION.
!*      if (LEXT) GO TO 1006
dxm = 0.q0
sumh = 0.q0
sumo = 0.q0
...
sumh16=sumh
dxm16=dxm
write(*, 101) dxm16, im, sumh16, e16, chk16
write(4, 101) dxm16, im, sumh16, e16, chk16
101 format(' DX =', d10.3, i4,'  DE =', d12.5, d32.25, d12.3)
return
end
!*******************************************************
...
!
      ...
      write(date, 1001) nm, nd, ny
 1001 format(i2.2,'/', i2.2,'/', i2.2)
      return
      end
subroutine dqwritx(n, x)
   ...
   read(2,'(a)') buffer
   do i=45, 50
      if (buffer(i:i).ne.buffer(i+1:i+1)) go to 10
   end do
   go to 11

10 if (n.ne.6) write(*,*) 'n =', n
   call dqwrite(6, x)
